"""
utils/database.py
SQLite database manager for OTT support conversations.
"""
import sqlite3
import os
...
def save_message(session_id: str, role: str, content: str,
                 language: str = "en", mode: str = "chat", provider: str = "gemini"):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO messages (session_id, role, content, language, mode, provider) VALUES (?, ?, ?, ?, ?, ?)",
        (session_id, role, content, language, mode, provider)
    )
    conn.commit()
    conn.close()
...
def get_all_messages_flat():
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT id, session_id, role, content, language, mode, provider, timestamp
        FROM messages ORDER BY id DESC LIMIT 1000
    """)
    rows = cursor.fetchall()
    conn.close()
    return [dict(r) for r in rows]
